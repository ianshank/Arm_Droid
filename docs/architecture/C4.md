...
real MakerWorld 6-DoF arm over ESP32 JSON-over-UART. All component boundaries are expressed as
...
graph TD
    ...
    subgraph cli ["CLI / public API"]
        main["armdroid.main\ncli_entry()\nargparse dispatch"]
        api["armdroid.api (future)\nREST / gRPC surface"]
    end
...
`pyproject.toml` — no fork required. See [ADR-0003](ADR/ADR-0003-registry-pattern.md).
...
opened to the MakerWorld ESP32 arm. The orchestrator itself does zero construction — it only stores and
